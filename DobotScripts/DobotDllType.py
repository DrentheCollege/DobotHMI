...
def SetDeviceWithL(api, isWithL, version=0, isQueued=0):
    ...
    while(True):
        print(tempSlaveId)
        result = api.SetDeviceWithL(c_bool(isWithL), c_uint8(version), c_bool(isQueued), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceID(api):
    ...
    timeout = False
    while(True):
        result = api.GetDeviceID(byref(deviceID))
        if result != DobotCommunicate.DobotCommunicate_NoError:
...
def SetColorSensor(api, isEnable, colorPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetColorSensor(enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensor(api,  isEnable, infraredPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetInfraredSensor(enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceVersionEx(api):       #2019.6.25 song 控制盒+Magician Lite时，获取控制盒的版本
    ...
    if masterDevType == DevType.Conntroller and slaveDevType == DevType.MagicianLite:
        ...
        while(True):
            result = api.GetDeviceVersion(byref(deviceVersion1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetUpgradeFWReadyCmd(api,fwSize, md5):
    ...
    # # 只发送给主设备
    # result = api.SetUpgradeFWReadyCmd(byref(upgradeFWReadyCmd))
    # return result
...
def GetUpgradeFWReadyCmd(api,fwSize, md5):
    ...
    if masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle):
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetUpgradeFWReadyCmd(byref(upgradeFWReadyCmd), byref(isUpgrade))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetColorSensorExt(api, isEnable, colorPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetColorSensor(enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensorExt(api,  isEnable, infraredPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetInfraredSensor(enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedColorSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedColorSensor(byref(r),  byref(g),  byref(b), byref(Cct))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedColorSensorExt(api, SeeedPort,isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedColorSensor(port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedDistanceSensorExt(api, SeeedPort):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedDistanceSensor(port, byref(distance))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedTempSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedTempSensor(port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedTempSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedTempSensor(byref(tem),  byref(hum))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedLightSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedLightSensor(port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedLightSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.GetSeeedLightSensor(byref(lux))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedRgbExt(api, SeeedPort, Rgb, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while(True):
        result = api.SetSeeedRgb(port, rgb, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetServoAngle(api, servoId, angle, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetServoAngle(isQueued, c_uint8(servoId), c_float(angle), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetServoAngle(api, servoId):
    angle = c_float(0)
    while(True):
        result = api.GetServoAngle( c_uint8(servoId) ,byref(angle))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while(True):
        result = api.SetLSpeedRatio(isQueued, c_uint8(paramsMode), c_uint8(speedRatio), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetLSpeedRatio(api, paramsMode):
    speedRatio = c_uint8(0)
    while(True):
        result = api.GetLSpeedRatio(c_uint8(paramsMode), byref(speedRatio))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetProgbar(api, progbar):
    while(True):
        result = api.SetProgbar(c_uint8(progbar))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
